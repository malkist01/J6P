...
# ================= PATH =================
ROOTDIR=$(pwd)
OUTDIR="$ROOTDIR/out/arch/arm/boot"
...
clone_anykernel() {
    if [ ! -d "$ANYKERNEL_DIR" ]; then
        ...
        git clone -b master https://github.com/malkist01/AnyKernel3.git "$ANYKERNEL_DIR" || exit 1
    fi
}
...
build_kernel() {
    ...
    rm -rf out
    make O=out ARCH=arm j6primelte_defconfig || {
        get_toolchain_info
